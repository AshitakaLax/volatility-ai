...
casually.
  ...
  Join semantics:      session-over-session percentage change in the
                       ...
                       engine/tests/test_implied_vol_signal.py rather than
...
def test_nothing_branches_on_the_macro_fields_outside_the_confirmed_consumer():
    ...
    for path in [p for pkg in ("engine", "research") for p in (REPO_ROOT / pkg).rglob("*.py")]:
        if path.name == Path(CONFIRMED_CONSUMER_MODULE).name:
            continue
        # PRODUCTION code only. Since the tests/ -> per-section split,
        # engine/tests/ and research/tests/ sit INSIDE the trees this
        # scans, and their own asserts about the macro fields (this file
        # included, were it under engine/ or research/) are exactly the
        # false positive this skip exists to prevent -- a test branching
        # on is_macro_event_day to check its VALUE is not "conditional
        # logic that now reads it" in the sense this test guards against.
