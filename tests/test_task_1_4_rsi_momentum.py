...
def test_rsi_momentum_waits_for_warmup():
    ...
    for price in [100.0, 99.0, 98.0]:
        strategy.record_tick(price)
    assert strategy.calculate_trade_value(100000.0, 98.0, 0.0) == 0.0
...
def test_rsi_momentum_reduces_allocation_when_rsi_is_overbought():
    ...
    for price in [100.0, 101.0, 102.0, 103.0, 104.0]:
        strategy.record_tick(price)
    value = strategy.calculate_trade_value(100000.0, 104.0, 0.0)
...
def test_rsi_momentum_is_deterministic_for_same_tick_sequence():
    ...
    for price in prices:
        first.record_tick(price)
        second.record_tick(price)
    assert math.isclose(
        first.calculate_trade_value(100000.0, prices[-1], 0.0),
        second.calculate_trade_value(100000.0, prices[-1], 0.0),
        rel_tol=0.0,
        abs_tol=1e-12,
    )
