...
The trading kernel: what actually executes and records trades, and the
invariants that protect real money. It knows **nothing** about any
concrete sizing algorithm — those live in `research/` and reach the
engine through the `SizingStrategy` port in `core/sizing.py`.

Dependency rule, enforceable and currently true: `engine/` never imports
`research/`, `server/`, `fidelity_gateway/` or `tools/` at module scope.
Everything above may import `engine/`. `core/` is layer 0 and imports no
other engine package at import time.

| Package | Key modules | Owns |
|---|---|---|
| `core/` | `config.py`, `sizing.py`, `market_context.py`, `validation.py`, `ledger.py`, `persistence.py`, `audit.py`, `secrets.py`, `exceptions.py`, `idempotency.py` | `BacktestConfig` (the single source of truth, validated front-loaded), the `SizingStrategy` **port**, the `MarketContext`/`SimulationResult` DTOs, lot-based position tracking, durable SQLite state, the `TradingSystemError` hierarchy |
| `trading/` | `decision_cycle.py`, `no_loss_guard.py`, `risk_manager.py`, `live_trading_loop.py`, `runtime_lifecycle.py` | the canonical per-bar decision sequence, **shared by backtest and live** — see below |
| `execution/` | `order_lifecycle.py`, `order_management_system.py`, `reconciliation.py`, `fill_accounting.py`, `live_execution.py`, `cost_models.py` | order state machine, broker-status mapping, local-vs-broker reconciliation, transaction-cost models (pure — compute and return, never mutate) |
| `data/` | `historical_data.py`, `alpaca_market_data.py`, `data_validation.py`, `tick_validation.py`, `intraday_profile.py`, `implied_vol_signal.py`, `*_calendar.py` | bar fetch/validation (Alpaca intake and live polling; a fetched CSV becomes backtest-visible only once `tools/build_warehouse.py --ingest` loads it), session structure, event/earnings/FOMC calendars |
| `warehouse/` | `connection.py`, `schema.py`, `hashing.py`, `ingest.py`, `duckdb_sink.py`, `queries.py`, `bars.py` | the DuckDB/Polars analytical warehouse: bars, sweep results, macro series, `ASOF` joins — see caveat below. **`bars.py` is the only read path a simulation uses.** |
...
`research/strategies/strategy_registry.py` maps `strategy_id` → `SizingStrategy`
...
## `research/ml/` is research, not a trading input — by default

`research/ml/reachability_sizing.py` changes only *how much* a confirmed grid buy
...
**Causal-transform rule, everywhere in `research/ml/`:** every rolling statistic is
...
`research/ml/rolling.py`'s `IncrementalBarFeatures` exists specifically because a
...
## `warehouse/` is the second optional-dependency package

`requirements.txt`'s rule is that the live loop and the Raspberry Pi must
never need an optional dependency to start, and that only `research/ml/` may
