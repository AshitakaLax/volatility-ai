...
def test_only_one_no_loss_comparison_exists_in_the_codebase():
    ...
    for path in [p for pkg in ("engine", "research") for p in (REPO_ROOT / pkg).rglob("*.py")]:
        ...
        # PRODUCTION code only. The tests/ -> per-section split put
        # engine/tests/ and research/tests/ INSIDE the trees this walks,
        # so without this a test file merely documenting or asserting
        # against the forbidden comparison (in a docstring, a fixture
        # string, an expected-error message) would count as a duplicate.
