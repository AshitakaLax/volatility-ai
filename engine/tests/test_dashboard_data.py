...
from engine.trading.live_trading_loop import _META_CASH, _META_UNSETTLED
from engine.trading.risk_manager import HALT_REASON_KEY, HALT_STATE_KEY
...
DASHBOARD_DATA = REPO_ROOT / "engine" / "data" / "dashboard_data.py"
DASHBOARD_UI = REPO_ROOT / "engine" / "ui" / "dashboard.py"
...
def test_a_halt_is_shown_as_an_error_not_a_footnote(tmp_path, monkeypatch):
    from engine.trading.live_trading_loop import _META_CASH
...
def test_drawdown_uses_the_persisted_high_water_mark(store, tmp_path):
    ...
    from engine.trading.live_trading_loop import _META_PEAK_EQUITY
...
def test_the_loops_own_tick_price_is_read_back(store, tmp_path):
    ...
    from engine.trading.live_trading_loop import _META_LAST_TICK
...
def demo_with_tick(tmp_path):
    ...
    from engine.trading.live_trading_loop import _META_LAST_TICK, _META_PEAK_EQUITY
