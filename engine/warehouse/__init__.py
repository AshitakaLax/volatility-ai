...
This is the second subpackage (after research/ml/) exempted from
...
That is why the sink Protocol lives in research/optimization/result_sink.py
...
def __getattr__(name: str):
    """Lazily re-export, so `import engine.warehouse` alone does not pull
    in duckdb/polars. Only hashing is dependency-free; the rest is
    resolved on first use."""
    if name == "parameter_hash":
        from engine.warehouse.hashing import parameter_hash
    ...
    if name in ("initialize",):
        from engine.warehouse.schema import initialize

        return initialize
    if name == "DuckDBResultSink":
        from engine.warehouse.duckdb_sink import DuckDBResultSink
