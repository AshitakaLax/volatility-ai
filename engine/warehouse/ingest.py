...
from engine.warehouse.connection import EXECUTIONS_DIR, EXTERNAL_DIR, OHLCV_DIR, lake_path

logger = logging.getLogger(__name__)

# The canonical bar schema, matching engine/data/historical_data.py's
...
def dataset_version(csv_path: str | Path) -> str:
    ...
    Prefers the sha256 that engine/data/historical_data.py already wrote
...
def suspect_bar_events(meta_json_path: str | Path, ticker: str) -> list[dict]:
    ...
    engine/data/data_validation.py flags any single-bar move over 15% and
