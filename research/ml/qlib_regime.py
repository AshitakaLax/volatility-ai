...
~600us/bar research/ml/reachability_sizing.py records for its per-bar
...
backtest/live parity that engine/trading/decision_cycle.py exists to
...
for all of research/ml/, expressed structurally: there is no shift to
forget, because the future bar is what triggers the computation.

--------------------------------------------------------------------
WHY NOT REUSE IncrementalBarFeatures

research/ml/rolling.py is minute-bar-specific in two places that would fail
...
# The volatility block from research/ml/features.py -- VIX term structure,
# VVIX, SKEW, VXN, RVX, OVX, GVZ, SPY realized vol. Same 15 columns
# research/ml/live_features.py uses, and for the same measured reason
...
class _WilderRSI:
    ...
    research/strategies/sizing_indicators.py, which tracks price ticks for a
...
class DailyRegimeFeatures:
    ...
    as research/ml/rolling.IncrementalBarFeatures.
    """
    ...
    def _rvol(self, window: int) -> float:
        ...
        if recent.size < 2:
            return float("nan")
        # ddof=1 matches research/ml/features.py's pandas .std() default, so a
...
def _load_booster(path: Path) -> tuple[Any, dict] | None:
    ...
    try:
        ...
        # Lazy, exactly as research/ml/reachability_sizing.py does it: merely
...
class RegimeInferenceSource:
    ...
    def __init__(
        ...
        *,
        ...
        # It is the same argument research/ml/live_features.py makes for
    ...
    def _predict(self, timestamp: datetime, vector: dict[str, float]) -> None:
        ...
        if self._crash_booster is not None:
            ...
            # research/ml/reachability_sizing.py records: this is ONE row, so
    ...
    def _warm_from_history(self, first_session: date) -> None:
        ...
        "look it up," not "skip it." See engine/warehouse/bars.py's own
