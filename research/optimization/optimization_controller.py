...
from engine.core.validation import validate_run_sweep_config
from engine.data import data_validation
from engine.data.earnings_calendar import EARNINGS_REACTION_DATES
from engine.data.fomc_calendar import EASTERN_TZ as _EASTERN_TZ
from engine.data.fomc_calendar import FOMC_DECISION_DATES
from engine.data.intraday_profile import SESSION_MINUTES as _SESSION_MINUTES
from engine.data.intraday_profile import SESSION_OPEN_MINUTE as _SESSION_OPEN_MINUTE
from engine.execution.cost_models import TransactionCostModel, ZeroCostModel
...
from research.analysis.performance_analyzer import (
    PerformanceAnalyzer,
    annual_returns,
    curve_metrics,
    trade_metrics,
)
from research.optimization import intraday_validation
...
def _run_one_combination(
    ...
    try:
        ...
        # is trained on ONE ticker's price history (research/ml/
...
class OptimizationController:
    ...
    def _load_event_table(self):
        ...
        """
        try:
            from engine.data.event_calendar import EarningsEventTable
    ...
    def _implied_vol_series(self):
        ...
        if not self._implied_vol_series_loaded:
            from engine.data.implied_vol_signal import load_implied_vol_change
    ...
    def _implied_vol_change(self):
        ...
        if self._implied_vol_change_cache is None:
            from engine.data.implied_vol_signal import changes_for_index
    ...
    def run_sweep(
        ...
        :param result_sink: Optional SweepResultSink
            (research/optimization/result_sink.py) handed every finished
