...
| `data/` | `historical_data.py`, `alpaca_market_data.py`, `data_validation.py`, `tick_validation.py`, `*_calendar.py` | bar fetch/validation (Alpaca intake and live polling; a fetched CSV becomes backtest-visible only once `tools/build_warehouse.py --ingest` loads it), event/earnings/FOMC calendars |
