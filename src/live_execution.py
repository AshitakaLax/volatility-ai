...
class LiveExecutionLoop:
    ...
    def start(self) -> None:
        ...
        if self.state_store is not None:
            ...
            self._fill_state = {order_id: FillCursor(cumulative_qty=qty, cumulative_notional=notional) for order_id, (qty, notional) in self.state_store.load_fill_cursors().items()}
    ...
    def shutdown(self, *, settle: Callable[[], bool] | None = None, max_wait_seconds: float = 30.0) -> RuntimeState:
        ...
        self.persist_state()
        self._record_audit("STARTUP_SHUTDOWN", StartupShutdownPayload(deployment_id=getattr(self.config, "deployment_id", "unknown"), runtime_state=RuntimeState.SHUTTING_DOWN.value, reconciliation_result=f"settled={settled}"), event_id=f"shutdown:{datetime.now(timezone.utc).isoformat()}")
        if not settled:
            self.reconciliation_required = True
            self._transition(RuntimeState.RECONCILIATION_REQUIRED)
            return self.runtime_state
    ...
    def evaluate_circuit_breaker(self, drawdown: float) -> bool:
        ...
        if halted != was_halted:
            self._record_audit("RISK_HALT", RiskHaltPayload(halt_reason="drawdown_exceeded" if halted else "reset", previous_state="halted" if was_halted else "active", new_state="halted" if halted else "active"))
    ...
    def validate_tick(self, price: float) -> bool:
        ...
        if price <= 0:
            ...
            return False
        if self._last_known_good_price is not None:
            move = abs(price / self._last_known_good_price - 1.0)
    ...
    def decision_cycle(self, context: MarketContext, *, step: float, last_buy_price: float) -> LiveDecision:
        ...
        market_event = self._record_canonical_audit("MARKET_CONTEXT", lambda _event_id: MarketContextPayload(timestamp=context.timestamp.isoformat(), symbol=symbol, OHLCV={"open": context.open, "high": context.high, "low": context.low, "close": context.close, "volume": 0.0}, bar_event_id=market_event_id), strategy_id=strategy_id, symbol=symbol, market_event_id=market_event_id, decision_type="MARKET_CONTEXT")
        ...
        strategy_event = self._record_canonical_audit("STRATEGY_DECISION", lambda event_id: StrategyDecisionPayload(decision_id=event_id, strategy_id=strategy_id, proposed_action="BUY" if triggered else "NONE", parameters={"proposed_trade_value": proposed, "step": step}), strategy_id=strategy_id, symbol=symbol, market_event_id=market_event_id, decision_type="STRATEGY_DECISION")
        ...
        self._record_canonical_audit("RISK_DECISION", lambda event_id: RiskDecisionPayload(decision_id=decision_id, allowed=(clamped > 0), reason="clamped" if clamped < proposed else ("halted" if self.circuit_breaker.halted else "allowed"), relevant_limits={"max_concurrent_lots": getattr(self.risk_manager, "max_concurrent_lots", None), "max_total_exposure": getattr(self.risk_manager, "max_total_exposure", None)}), strategy_id=strategy_id, symbol=symbol, market_event_id=market_event_id, decision_type="RISK_DECISION")
    ...
    def apply_sell_fill(self, order: Any, lot: InventoryLot, ledger: AssetLotLedger | None = None, cash: float = 0.0, *, execution_cost: float = 0.0, cost_model: TransactionCostModel | None = None) -> tuple[float, float]:
        ...
        audit_events = [
            AuditEvent(event_id=f"{event_id}:status", timestamp=datetime.now(timezone.utc).isoformat(), event_type="ORDER_STATUS", schema_version=1, deployment_id=getattr(self.config, "deployment_id", "unknown"), payload=dataclasses.asdict(OrderStatusPayload(intent_id=order_id, broker_order_id=order_id, status=str(getattr(order, "status", "FILLED")), cumulative_filled_qty=current_qty))),
            AuditEvent(event_id=event_id, timestamp=datetime.now(timezone.utc).isoformat(), event_type="FILL", schema_version=1, deployment_id=getattr(self.config, "deployment_id", "unknown"), payload=dataclasses.asdict(FillPayload(fill_id=event_id, order_id=order_id, incremental_fill_qty=new_qty, cumulative_filled_qty=current_qty, price=new_notional / new_qty, fees=execution_cost, timestamp=datetime.now(timezone.utc).isoformat()))),
            AuditEvent(event_id=f"{event_id}:ledger", timestamp=datetime.now(timezone.utc).isoformat(), event_type="LEDGER_MUTATION", schema_version=1, deployment_id=getattr(self.config, "deployment_id", "unknown"), payload=dataclasses.asdict(LedgerMutationPayload(event_id=event_id, lot_id=lot.order_id, mutation_type="close_lot" + ("_violation" if fill_violation else ""), quantity_delta=-new_qty, cash_delta=net_proceeds))),
        ]
        if self.state_store is not None:
            claimed, _ = self.state_store.apply_fill_transaction(event_id=event_id, ledger=ledger, order_id=order_id, cumulative_qty=current_qty, cumulative_notional=current_notional, cursor=cursor, mutation=lambda: ledger.close_lot(lot, sell_qty=new_qty, execution_price=new_notional / new_qty, completed=False), audit_events=audit_events)
            if not claimed:
                return float(cash), 0.0
        else:
            ...
            for audit_event in audit_events:
                self._record_audit(audit_event.event_type, audit_event.payload, event_id=audit_event.event_id)
    ...
    def apply_buy_fill(self, order: Any, symbol: str, profit_target: float, ledger: AssetLotLedger | None = None, cash: float = 0.0, *, execution_cost: float = 0.0) -> tuple[float, InventoryLot | None]:
        ...
        audit_events = [
            AuditEvent(event_id=f"{event_id}:status", timestamp=datetime.now(timezone.utc).isoformat(), event_type="ORDER_STATUS", schema_version=1, deployment_id=getattr(self.config, "deployment_id", "unknown"), payload=dataclasses.asdict(OrderStatusPayload(intent_id=order_id, broker_order_id=order_id, status=str(getattr(order, "status", "FILLED")), cumulative_filled_qty=current_qty))),
            AuditEvent(event_id=event_id, timestamp=datetime.now(timezone.utc).isoformat(), event_type="FILL", schema_version=1, deployment_id=getattr(self.config, "deployment_id", "unknown"), payload=dataclasses.asdict(FillPayload(fill_id=event_id, order_id=order_id, incremental_fill_qty=new_qty, cumulative_filled_qty=current_qty, price=new_avg, fees=execution_cost, timestamp=datetime.now(timezone.utc).isoformat()))),
            AuditEvent(event_id=f"{event_id}:ledger", timestamp=datetime.now(timezone.utc).isoformat(), event_type="LEDGER_MUTATION", schema_version=1, deployment_id=getattr(self.config, "deployment_id", "unknown"), payload=dataclasses.asdict(LedgerMutationPayload(event_id=event_id, lot_id=order_id, mutation_type="register_buy", quantity_delta=new_qty, cash_delta=-(new_notional + execution_cost)))),
        ]
        if self.state_store is not None:
            claimed, lot = self.state_store.apply_fill_transaction(event_id=event_id, ledger=ledger, order_id=order_id, cumulative_qty=current_qty, cumulative_notional=current_notional, cursor=cursor, mutation=lambda: ledger.register_buy(order_id, symbol, new_avg, new_qty, profit_target), audit_events=audit_events)
            if not claimed:
                return float(cash), None
        else:
    ...
    def submit_buy(self, decision: LiveDecision) -> Any:
        ...
        intent_event = self._record_order_intent(decision_id=decision.decision_id or "", symbol=symbol, side="BUY", quantity=decision.clamped_trade_value, limit_price=None, market_event_id=market_event_id)
    ...
    def submit_sell(self, qty: float, target_price: float, decision_id: str = "", *, lot: InventoryLot | None = None, cost_model: TransactionCostModel | None = None, market_event_id: str | int | None = None) -> Any:
        ...
        intent_event = self._record_order_intent(decision_id=decision_id, symbol=symbol, side="SELL", quantity=qty, limit_price=target_price, market_event_id=resolved_market_event_id)
        if intent_event is not None:
            intent_id = intent_event.event_id
        else:
            intent_id = generate_event_id()
