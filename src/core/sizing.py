...
This ABC used to sit in src/strategies/size_calculators.py alongside
the concrete strategies that implement it. That meant the two engine
modules which merely CALL a strategy --
src/execution/live_execution.py and src/trading/decision_cycle.py --
had to import src.strategies to name the type, so the engine imported
...
strategies in src/strategies/ and src/ml/ are the adapters that
implement it, and nothing in the engine needs to know they exist --
src/strategies/strategy_registry.py is the one place that does.

size_calculators.py re-exports this name, so the ~28 modules and tests
that do `from src.strategies.size_calculators import SizingStrategy`
